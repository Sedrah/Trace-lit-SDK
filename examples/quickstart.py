#!/usr/bin/env python3
"""
Trace-lit quickstart — sends one test trace and confirms delivery.

Usage:
    python examples/quickstart.py --broker app.trace-lit.com:9093 --key sk-demo

Or after pip install:
    python -m trace_lit.quickstart --broker app.trace-lit.com:9093 --key sk-demo
"""
import argparse
import sys
import threading
...
def main() -> None:
    ...
    try:
        ...
        _fail('tracelit-sdk not installed. Run: pip install "tracelit-sdk[kafka]"')

    # ── 2. Build a test trace ────────────────────────────────────────────────
    trace_id = uuid4()
    event = TraceEvent(
        trace_id=trace_id,
        span_id=uuid4(),
    ...
    # ── 3. Produce with a delivery callback for synchronous confirmation ─────
    # We bypass the SDK's async batching emitter here so we can block on
    # flush() and know for certain whether the broker accepted the message.
